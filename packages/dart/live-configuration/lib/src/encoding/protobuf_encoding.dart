...
class ProtobufEncoding {
  ...
  static dynamic _extractValue(ConfigurationEntryValue value) {
    ...
    switch(valueType) {
      ...
      case ConfigurationEntryValue_Kind.durationValue:
        return Duration(seconds: value.durationValue.seconds.toInt(), microseconds: (value.durationValue.nanos / 1000).floor());
      ...
      default: throw Exception('Invalid type $valueType');
    }
  }
  ...
  static ConfigurationEntryValue _getConfigurationEntryValue(dynamic value, ConfigurationEntryValueType valueType) {
    switch(valueType) {
      ...
      case ConfigurationEntryValueType.ConfigurationEntryValueType_DURATION:
        ...
        return ConfigurationEntryValue(durationValue: protobuf.Duration(seconds: Int64(duration.inSeconds % 60), nanos: (duration.inMilliseconds % 60) * 1000));

      case ConfigurationEntryValueType.ConfigurationEntryValueType_TIMESTAMP:
        return ConfigurationEntryValue(timestampValue: Timestamp.fromDateTime(value));

      case ConfigurationEntryValueType.ConfigurationEntryValueType_LIST:
        var iterable = value as Iterable;
        return ConfigurationEntryValue(listValue: ConfigurationEntryListValue(
          values: iterable.map((e) => _getConfigurationEntryValueWithoutValueType(e)
        )));

      case ConfigurationEntryValueType.ConfigurationEntryValueType_JSON:
        return ConfigurationEntryValue(
          mapValue: ConfigurationEntryMapValue(
            fields: { for (var e in value.entries.map((e) => MapEntry(e.key, _getConfigurationEntryValueWithoutValueType(e.value)))) e.key as String : e.value }
          )
        );

      default: return ConfigurationEntryValue(nullValue: ConfigurationEntryValue_NullValue.NULL_VALUE_NULL);
    }
  }
  ...
  static ConfigurationEntryValue _getConfigurationEntryValueWithoutValueType(dynamic value) {
    if(value == null) {
      ...
      return ConfigurationEntryValue(listValue: ConfigurationEntryListValue(
        values: value.map((e) => _getConfigurationEntryValueWithoutValueType(e))
      ));
      ...
      return ConfigurationEntryValue(durationValue: protobuf.Duration(seconds: Int64(value.inSeconds % 60), nanos: (value.inMilliseconds % 60) * 1000));
    } else {
